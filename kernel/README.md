...
本项目的语言特性目前的实现方案是依赖Clang语言特性的，但所有语言特性将约束在`support`这一子目录下的`aos.kernel.support`模块。如果需要进行移植，对`include/support`下的头文件与`support`目录下的实现进行修改，提供替代方案即可。其他模块只允许调用由`aos.kernel.support`实现或包装的函数。