...
final class Parser
{
	...
	{
		boolean isNot=false;
		final LinkedList<CharPredicate> and=new LinkedList<>();
		final LinkedList<CharPredicate> or=new LinkedList<>();
		if(lookahead()=='^')
		{
			next();
			isNot=true;
		...
		while(true)
		{
			switch(lookahead()){
				...
				case '&':
					...
					{
						p=or.removeFirst();
						for(final CharPredicate pred:or)
						{
				...
				case ']':
					...
					else
					{
						...
						and.add(p);
						p=and.removeFirst();
						for(final CharPredicate pred:and)
						{
	...
	private Digraph closure(Digraph digraph)
	{
		return switch(lookahead()){
			case '*'->{
				// 0到无穷。
				next();
				yield digraph.closure();
			}
			case '+'->{
				// 1到无穷。
				next();
				yield digraph.concat(digraph.closure());
			}
			case '?'->{
				// 0到1。
				next();
				yield digraph.union(Digraph.transform(EPSILON));
			}
			case '{'->{
				next();
				yield closureRange(digraph);
			}
			default	->digraph;
		};
	}
	...
	private CharPredicate escape()
	{
		...
		return switch(lookahead()){
			...
			case 'H'->{
				// 非水平空白。
				next();
				yield CharPredicate.not(HSPACE);
			}
			case 'v'->{
				// 竖直空白。
				next();
			...
			case 'V'->{
				// 非竖直空白。
				next();
				yield CharPredicate.not(VSPACE);
			}
			case 's'->{
				// 一般空白。
				next();
			...
			case 'S'->{
				// 非一般空白。
				next();
				yield CharPredicate.not(WHITE_SPACE);
			}
			case 'w'->{
				// 单词。
				next();
	...
	{
		final LinkedList<Digraph> unions=new LinkedList<>();
		final LinkedList<Digraph> concats=new LinkedList<>();
		Digraph d;
		CharPredicate p;
		while(true)
		{
			switch(lookahead()){
				...
				case '|':
					...
					else
					{
						...
						for(final Digraph digraph:concats)
						{
							d=d.concat(digraph);
						}
						concats.clear();
				...
				case EOS:
					if(isStart)
					{
						...
						unions.add(d);
						d=unions.removeFirst();
						for(final Digraph digraph:unions)
						{
							d=d.union(digraph);
						}
						return d;
				...
				switch(lookahead()){
					...
					case '}':
						...
						return switch(n.toString()){
							...
							case "Space"	->SPACE;
							case "d"		->NUMBER;
							case "D"		->CharPredicate.not(NUMBER);
							case "s"		->SPACE;
							case "S"		->CharPredicate.not(SPACE);
							case "w"		->WORD;
							case "W"		->CharPredicate.not(WORD);
							default			->throw new IllegalArgumentException("分析转义序列[\\p{name}]，在下标%d时发现不存在的字符类名[%s]。".formatted(cursor,n.toString()));
						};
					default:
						n.appendCodePoint(lookahead());
						next();
						...
						switch(text){
							...
							default:
								...
								else if(text.startsWith("general_category="))
								{
									...
									throw new IllegalArgumentException("分析转义序列[\\U{name}]，在下标%d时发现不存在的Unicode类别[%s]。".formatted(cursor,text));
								}
	...
	static String getParam(String text,int type)
	{
		switch(type){
			...
			case 2:
				if(text.startsWith("[^")||text.contains("&")||!text.startsWith("["))
				{
					return text;
				}
				else
	...
	static CharPredicate getPredicate(String category)
	{
		return switch(category){
			case "Cn"	->CharPredicate.create(1<<Character.UNASSIGNED,"Cn");
			case "Lu"	->CharPredicate.create(1<<Character.UPPERCASE_LETTER,"Lu");
			case "Ll"	->CharPredicate.create(1<<Character.LOWERCASE_LETTER,"Ll");
			case "Lt"	->CharPredicate.create(1<<Character.TITLECASE_LETTER,"Lt");
			case "Lm"	->CharPredicate.create(1<<Character.MODIFIER_LETTER,"Lm");
			case "Lo"	->CharPredicate.create(1<<Character.OTHER_LETTER,"Lo");
			case "Mn"	->CharPredicate.create(1<<Character.NON_SPACING_MARK,"Mn");
			case "Me"	->CharPredicate.create(1<<Character.ENCLOSING_MARK,"Me");
			case "Mc"	->CharPredicate.create(1<<Character.COMBINING_SPACING_MARK,"Mc");
			case "Nd"	->CharPredicate.create(1<<Character.DECIMAL_DIGIT_NUMBER,"Nd");
			case "Nl"	->CharPredicate.create(1<<Character.LETTER_NUMBER,"Nl");
			case "No"	->CharPredicate.create(1<<Character.OTHER_NUMBER,"No");
			case "Zs"	->CharPredicate.create(1<<Character.SPACE_SEPARATOR,"Zs");
			case "Zl"	->CharPredicate.create(1<<Character.LINE_SEPARATOR,"Zl");
			case "Zp"	->CharPredicate.create(1<<Character.PARAGRAPH_SEPARATOR,"Zp");
			case "Cc"	->CharPredicate.create(1<<Character.CONTROL,"Cc");
			case "Cf"	->CharPredicate.create(1<<Character.FORMAT,"Cf");
			case "Co"	->CharPredicate.create(1<<Character.PRIVATE_USE,"Co");
			case "Cs"	->CharPredicate.create(1<<Character.SURROGATE,"Cs");
			case "Pd"	->CharPredicate.create(1<<Character.DASH_PUNCTUATION,"Pd");
			case "Ps"	->CharPredicate.create(1<<Character.START_PUNCTUATION,"Ps");
			case "Pe"	->CharPredicate.create(1<<Character.END_PUNCTUATION,"Pe");
			case "Pc"	->CharPredicate.create(1<<Character.CONNECTOR_PUNCTUATION,"Pc");
			case "Po"	->CharPredicate.create(1<<Character.OTHER_PUNCTUATION,"Po");
			case "Sm"	->CharPredicate.create(1<<Character.MATH_SYMBOL,"Sm");
			case "Sc"	->CharPredicate.create(1<<Character.CURRENCY_SYMBOL,"Sc");
			case "Sk"	->CharPredicate.create(1<<Character.MODIFIER_SYMBOL,"Sk");
			case "So"	->CharPredicate.create(1<<Character.OTHER_SYMBOL,"So");
			case "Pi"	->CharPredicate.create(1<<Character.INITIAL_QUOTE_PUNCTUATION,"Pi");
			case "Pf"	->CharPredicate.create(1<<Character.FINAL_QUOTE_PUNCTUATION,"Pf");
			case "L"	->CharPredicate.create(1<<Character.UPPERCASE_LETTER|1<<Character.LOWERCASE_LETTER|1<<Character.TITLECASE_LETTER|1<<Character.MODIFIER_LETTER|1<<Character.OTHER_LETTER,"L");
			case "M"	->CharPredicate.create(1<<Character.NON_SPACING_MARK|1<<Character.ENCLOSING_MARK|1<<Character.COMBINING_SPACING_MARK,"M");
			case "N"	->CharPredicate.create(1<<Character.DECIMAL_DIGIT_NUMBER|1<<Character.LETTER_NUMBER|1<<Character.OTHER_NUMBER,"N");
			case "Z"	->CharPredicate.create(1<<Character.SPACE_SEPARATOR|1<<Character.LINE_SEPARATOR|1<<Character.PARAGRAPH_SEPARATOR,"Z");
			case "C"	->CharPredicate.create(1<<Character.CONTROL|1<<Character.FORMAT|1<<Character.PRIVATE_USE|1<<Character.SURROGATE|1<<Character.UNASSIGNED,"C");
			case "P"	->CharPredicate.create(1<<Character.DASH_PUNCTUATION|1<<Character.START_PUNCTUATION|1<<Character.END_PUNCTUATION|1<<Character.CONNECTOR_PUNCTUATION|1<<Character.OTHER_PUNCTUATION|1<<Character.INITIAL_QUOTE_PUNCTUATION|1<<Character.FINAL_QUOTE_PUNCTUATION,"P");
			case "S"	->CharPredicate.create(1<<Character.MATH_SYMBOL|1<<Character.CURRENCY_SYMBOL|1<<Character.MODIFIER_SYMBOL|1<<Character.OTHER_SYMBOL,"S");
			default		->null;
		};
	}
