 ...
 * 
 * <p>
 * 该包负责提供一种<s>NFA引擎</s>DFA引擎的正则分析器。
 * 
 * <p>
 * 已经实现功能与介绍如下面篇幅所述。
 * 
 ...
 * <li><code>[a-z\u0026a-c]</code> -
 * 字符a到z与字符a到c的交集（交集）。等价于“<code>[a-c]</code>”。</li>
 * <li><code>[a-c[d-f]\u0026c-e]</code> -
 * 字符a到c和字符d-f的并集与字符c到e的交集，其中字符d-f为一嵌套字符类。等价于“<code>[c-e]</code>”。</li>
 * </ol>
 * 字符类的主要语法已在上面举例。在字符类中，字符范围可以使用第二节所述的任何字符常量形式和后面所描述的预定义字符类。<br/>
 * 在字符类中，范围符“<code>-</code>”前后须有单个字符，且两字符大小关系不能颠倒，应小字符在前，大字符在后，
 * 除了字符颠倒情况以外范围符将变成一般字符；反向符“<code>^</code>”须在字符类开头才为反向符，否则为一般字符；
 * 交集符“<code>&</code>”在字符类开头（除掉反向符后）或在交集符后为一般字符，其余情况前后须有字符、范围或字符类通过无限个或运算连接的总字符类。<br/>
 * 集合运算优先级：先并集，后交集，最后确定是否取反。内部的嵌套字符类会提高嵌套字符类内部集合运算的优先级。
 * 
 * <h2>四、正则预定义字符类</h2>
 * 
 * 正则库预定义了以下字符类。
 ...
 * <li><code>\h</code> -
 * 一个水平空白符。等价于“<code>[ \t\x{A0}\x{1680}\x{180E}\x{2000}-\x{200A}\x{202F}\x{205F}\x{3000}]</code>”。</li>
 ...
 * <li><code>\w</code> -
 * 一个词汇字符。等价于“<code>[\U{Alpha}\U{gc=Mn}\U{gc=Me}\U{gc=Mc}\U{Digit}\U{gc=Pc}\U{IsJoin_Control}]</code>”。</li>
 ...
 * <li><code>\p{Alpha}</code> -
 * 一个字母。等价于“<code>[\p{Lower}\p{Upper}]</code>”。</li>
 * <li><code>\p{Digit}</code> - 一个十进制数字。等价于“<code>[0-9]</code>”。</li>
 * <li><code>\p{Alnum}</code> -
 * 一个字母或数字。等价于“<code>[\p{Alpha}\p{Digit}]</code>”。</li>
 * <li><code>\p{Punct}</code> -
 * 一个标点。等价于“<code>[!"#$%&'()*+,\-\./:;<=>?@\[\\\]^_`{|}~]|[\x{20}-\x{2F}\x{3A}-\x{40}\x{5B}-\x{60}\x{7B}-\x{7E}]</code>”。</li>
 * <li><code>\p{Graph}</code> -
 * 一个可见字符。等价于“<code>[\p{Alnum}\p{Punct}]</code>”。</li>
 * <li><code>\p{Print}</code> - 一个可打印字符。等价于“<code>[ \p{Graph}]</code>”。</li>
 * <li><code>\p{Blank}</code> - 一个空白。等价于“<code>[ \t]</code>”。</li>
 * <li><code>\p{Cntrl}</code> -
 * 一个控制字符。等价于“<code>[\x{0}-\x{1F}\x{7F}]</code>”。</li>
 * <li><code>\p{XDigit}</code> - 一个十六进制数字。等价于“<code>[0-9a-fA-F]</code>”。</li>
 * <li><code>\p{Space}</code> - 一个空白字符。等价于“<code>\p{s}</code>”。</li>
 * <li><code>\p{d}</code> -
 * 一个数字。对标“<code>\d</code>”。等价于“<code>[0-9]</code>”。</li>
 * <li><code>\p{D}</code> -
 * 一个非数字。对标“<code>\D</code>”。等价于“<code>[^0-9]</code>”。</li>
 * <li><code>\p{s}</code> -
 * 一个空白符。对标“<code>\s</code>”。等价于“<code>[ \t\n\x{B}\f\r]</code>”。</li>
 * <li><code>\p{S}</code> -
 * 一个非空白符。对标“<code>\S</code>”。等价于“<code>[^\s]</code>”。</li>
 * <li><code>\p{w}</code> -
 * 一个词汇字符。对标“<code>\w</code>”。等价于“<code>[a-zA-Z_0-9]</code>”。</li>
 * <li><code>\p{W}</code> -
 * 一个非词汇字符。对标“<code>\W</code>”。等价于“<code>[^\w]</code>”。</li>
 * </ol>
 ...
 * 
 * 以下举例表示如何引用Unicode的类别（category)、书写系统（script）和代码区段（block）。具体名称参照运行的Java虚拟机中{@link java.lang.Character
 * 字符类}支持的Unicode标准。特别注意，类别支持单字符大类别和双字符小类别。
 * <ol>
 * <li><code>\U{Lu}</code> - 一个大写字母（直接引用类别）。</li>
 * <li><code>\U{IsLl}</code> - 一个小写字母（“<code>Is</code>”前缀引用类别）。</li>
 * <li><code>\U{general_category=L}</code> -
 * 一个字母（“<code>general_category=</code>”前缀引用类别）。</li>
 ...
 * <li><code>\U{block=Mongolian}</code> -
 * 一个蒙古文（“<code>block=</code>”前缀引用代码区段）。</li>
 ...
 * <li><code>\U{Alnum}</code> -
 * 一个字母或数字。等价于“<code>[\U{IsAlphabetic}\U{IsDigit}]</code>”。</li>
 * <li><code>\U{Punct}</code> - 一个标点。等价于“<code>\U{IsPunctuation}</code>”。</li>
 * <li><code>\U{Graph}</code> -
 * 一个可见字符。等价于“<code>[^\U{IsWhite_Space}\U{gc=Cc}\U{gc=Cs}\U{gc=Cn}]</code>”。</li>
 * <li><code>\U{Print}</code> -
 * 一个可打印字符。等价于“<code>[\U{Graph}\U{Blank}&&[^\U{Cntrl}]]</code>”。</li>
 * <li><code>\U{Blank}</code> -
 * 一个空白。等价于“<code>[\U{IsWhite_Space}&&[^\U{gc=Zl}\U{gc=Zp}\x{A}-\x{D}\x{85}]]</code>”。</li>
 * <li><code>\U{Cntrl}</code> - 一个控制字符。等价于“<code>\U{gc=Cc}</code>”。</li>
 * <li><code>\U{XDigit}</code> -
 * 一个十六进制数字。等价于“<code>[[\U{gc=Nd}\U{IsHex_Digit}]]</code>”。</li>
